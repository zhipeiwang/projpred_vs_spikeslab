...
workflow_fun <- function(pos, conditions) {
  ...
  # ppvs_out <- run_projpred(refm_fit, K = 5, nterms_max = 11)
...
workflow_fun(1, remaining_conditions)
...
nworkers <- 5 # number of cores to use, max = 20
...
clusterExport(cl, varlist = c("remaining_conditions", "workflow_fun", "seed"),
              envir = environment())
...
system.time({
  out <- parLapplyLB(cl, 1:nrow(remaining_conditions), function(i) {
    workflow_fun(i, remaining_conditions)  # Only pass row index, not the entire data frame
  })
})
...
# saved_files <- list.files("output_study1/df_out/", full.names = FALSE)
# 
