...
fit_reference_model <- function(data, seed = 123) {
  ...
  return(list(refm_fit = refm_fit, refm_obj = refm_obj))
}
```
...
```{r}
run_cross_validation <- function(refm_obj, k_values, max_terms = 11, seed = 123) {
  cv_results <- list()
  for (K in k_values) {
    cv_name <- paste0(K, "-fold")
    cv_results[[cv_name]] <- cv_varsel(
      refm_obj,
      cv_method = "kfold",
      K = K,
      method = "forward",
...
```{r}
data_s1_small_r1 <- load_and_prepare_data("p50/sim_p50_t10.csv", sample_size = 100, replication = 1)
```
