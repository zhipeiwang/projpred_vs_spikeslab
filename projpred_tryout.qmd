...
```{r}
calculate_inclusion_metrics <- function(selected_predictors, true_predictors, total_predictors = 75) {
  # Identify irrelevant predictors
  irrelevant_predictors <- paste0("X", 1:15)[!(paste0("X", 1:total_predictors) %in% true_predictors)]  # Predictors that should be excluded

  # Calculate inclusion and exclusion counts
  true_inclusion <- sum(selected_predictors %in% true_predictors)   # Correctly included
  false_inclusion <- sum(!(selected_predictors %in% true_predictors))  # Incorrectly included
  false_exclusion <- sum(true_predictors %in% true_predictors & !(true_predictors %in% selected_predictors))  # Incorrectly excluded
  true_exclusion <- sum(!(irrelevant_predictors %in% selected_predictors))  # Correctly excluded

  # Calculate rates
  true_inclusion_rate <- true_inclusion / length(true_predictors)  # Proportion of true predictors correctly included
  false_inclusion_rate <- false_inclusion / length(irrelevant_predictors)  # Proportion of irrelevant predictors incorrectly included
  true_exclusion_rate <- true_exclusion / length(irrelevant_predictors)  # Proportion of irrelevant predictors correctly excluded
