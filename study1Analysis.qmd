...
df_out_files_study1 <- list.files("output_study1/ppvs/df_out/", pattern = "^df_out_", full.names = TRUE)
...
all_df_out_study1 <- bind_rows(lapply(df_out_files_study1, read_df_out))

# View structure of the final dataframe
glimpse(all_df_out_study1)
head(all_df_out_study1)
```

# ppvs convergence
```{r}
# stats for number of divergent transitions per replication
all_df_out_study1 %>%
...
# stats for perc of divergent transitions per replication
all_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
...
# take a look at the most problematic condition (highest mean_div)
all_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
  summarise(perc_div = unique(div) * 100 / 4000, .groups = "drop") %>%
  filter(N == 100, corr == 0.4, TE == "mixed") %>%
  arrange(desc(perc_div))

# problematic replications
all_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
  summarise(
    n_div = unique(div),
...
problematic_reps_study1 <- all_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
  summarise(
    n_div = unique(div),
    perc_div = unique(div) * 100 / 4000, 
...
nonconver_reps_study1 <- all_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
  summarise(n_div = unique(div),
            perc_div = unique(div) * 100 / 4000,
            n_Rhat = sum(Rhat >= 1.1),
...
```{r}
summary_na_study1 <- all_rates_long_study1 %>%
  anti_join(nonconver_reps_study1, by  = c("N", "corr", "TE", "r")) %>%
...
```{r}
ggplot(summary_na_study1, aes(x = heuristic, y = na_count, fill = factor(N))) +
...
ggplot(summary_na_study1, aes(x = heuristic, y = na_count, fill = factor(TE))) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(N ~ corr) +
  labs(
    title = "Number of NAs per Heuristic and Condition",
...
ggplot(summary_na_study1, aes(x = heuristic, y = na_count, fill = factor(corr))) +
  geom_bar(stat = "identity", position = "dodge") +
  facet_grid(N ~ TE) +
  labs(
    title = "Number of NAs per Heuristic and Condition",
...
get_true_predictors_study1 <- function(TE) {
  if (TE == "clustered") {
    return(paste0("X", 1:10))
  } else if (TE == "mixed") {
    return(paste0("X", seq(1, 50, 5)))
  } else {
    stop("Unexpected TE value!")
  }
}

compute_rates_from_df_study1 <- function(df, heuristics) {

  true_predictors <- get_true_predictors_study1(df$TE[1])
...
all_rates_long_study1 <- all_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
  group_split() %>%
  map_dfr(~ compute_rates_from_df_study1(.x, heuristics))

all_rates_long_study1 <- all_rates_long_study1 %>%
  mutate(heuristic = str_remove(heuristic, "^selected_pred_ppvs_"))

glimpse(all_rates_long_study1)
head(all_rates_long_study1)
```
# ssvs rates
```{r}
# List all ssvs_df_out files
ssvs_files_study1 <- list.files("output_study1/ssvs/ssvs_df_out/", pattern = "^ssvs_N", full.names = TRUE)

# Extract condition details from filenames
ssvs_conditions <- tibble(
  file = ssvs_files_study1,
  N = as.numeric(str_extract(ssvs_files_study1, "(?<=ssvs_N)\\d+")),
  corr = as.numeric(str_extract(ssvs_files_study1, "(?<=_corr)0(?:\\.\\d+)?")),
  TE = str_extract(ssvs_files_study1, "(?<=_TE)[a-z]+"),
  r = as.numeric(str_extract(ssvs_files_study1, "(?<=_r)\\d+"))
)
...
all_ssvs_df_out_study1 <- bind_rows(lapply(ssvs_files_study1, load_ssvs_df_out))
head(all_ssvs_df_out_study1)
```


```{r cache=TRUE}
compute_ssvs_rates_study1 <- function(df) {
...
all_ssvs_rates_study1 <- all_ssvs_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
  group_split() %>%
  map_dfr(compute_ssvs_rates_study1)

print(all_ssvs_rates_study1)

```

# combine rates
```{r}
# Add heuristic column to ssvs dataset
all_ssvs_rates_study1 <- all_ssvs_rates_study1 %>%
...
combined_rates_study1 <- bind_rows(all_rates_long_study1, all_ssvs_rates_study1)

# View structure to check
glimpse(combined_rates_study1)
head(combined_rates_study1)
```

# counts
```{r}
# counts, number of selected variables by heuristic
ppvs_counts_study1 <- all_df_out_study1 %>%
  anti_join(nonconver_reps_study1, by = c("N", "corr", "TE", "r")) %>%
  group_by(N, corr, TE, r) %>%
...
ssvs_counts_study1 <- all_ssvs_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
  summarise(n_selected = sum(selected_pred_ssvs == 1),
            .groups = "drop") %>%
  mutate(heuristic = "SSVS")

combined_counts_study1 <- bind_rows(ppvs_counts_study1, ssvs_counts_study1)
combined_counts_study1
```


# four rates different facets
```{r}
combined_rates_study1 %>%
...
```{r cache=TRUE}
assign_effect_sizes_study1 <- function(TE) {
  effect_sizes <- rep("null", 50)

  if (TE == "clustered") {
    true_indices <- 1:10  # First 10 predictors are true
  } else if (TE == "mixed") {
    true_indices <- seq(1, 50, by = 5)  # Every 5th predictor is true
  } else {
    stop("Unknown TE value")
  }

  # Define the pattern: large, medium, medium, small, small
  size_pattern <- rep(c("large", "medium", "medium", "small", "small"), length.out = length(true_indices))

  effect_sizes[true_indices] <- size_pattern
  return(effect_sizes)
}


compute_rates_by_effect_size_study1 <- function(df, heuristics) {
  results <- list()
  ...
  for (heuristic in heuristics) {
    ...
    effect_sizes <- assign_effect_sizes_study1(df$TE[1])
...
ppvs_rates_by_effect_size_study1 <- all_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
  group_split() %>%
  map_dfr(~ compute_rates_by_effect_size_study1(.x, heuristics))

ppvs_rates_by_effect_size_study1 <- ppvs_rates_by_effect_size_study1 %>%
  anti_join(nonconver_reps_study1, by = c("N", "corr", "TE", "r"))

ppvs_rates_by_effect_size_study1 <- ppvs_rates_by_effect_size_study1 %>%
  mutate(heuristic = str_remove(heuristic, "^selected_pred_ppvs_"))
ppvs_rates_by_effect_size_study1
```


# ssvs rates by effect size
```{r cache=TRUE}
compute_ssvs_rates_by_effect_size_study1 <- function(df) {
  # Get TE and corresponding true predictors
  true_predictors <- get_true_predictors_study1(df$TE[1])
  
  # Generate full variable list ("X1" to "X50")
  all_vars <- paste0("X", 1:50)
  
  # Define effect size pattern and assign to true predictors
  size_pattern <- c("large", "medium", "medium", "small", "small")
  pattern_repeats <- rep(size_pattern, length.out = length(true_predictors))
  effect_sizes <- setNames(rep("null", 50), all_vars)
  effect_sizes[true_predictors] <- pattern_repeats
  
  # Add effect size column
  df <- df %>%
    mutate(effect_size = effect_sizes[Variable])
...
all_ssvs_rates_by_effect_size_study1 <- all_ssvs_df_out_study1 %>%
  group_by(N, corr, TE, r) %>%
  group_split() %>%
  map_dfr(compute_ssvs_rates_by_effect_size_study1)
all_ssvs_rates_by_effect_size_study1
```

# combine rates by effect size
```{r}
# Combine both datasets
combined_rates_by_effect_size_study1 <- bind_rows(ppvs_rates_by_effect_size_study1, all_ssvs_rates_by_effect_size_study1)

# View structure to check
glimpse(combined_rates_by_effect_size_study1)
head(combined_rates_by_effect_size_study1)
```


# combined rates by effect size visualization
```{r}
combined_rates_by_effect_size_study1 %>%
...
p_rates_small_study1 <- combined_rates_by_effect_size_study1 %>%
  filter(!(heuristic %in% c("lower", "thres_lower", "thres_upper"))) %>%
  mutate(
  ...
  geom_boxplot(alpha = 0.7) +
  stat_summary(fun = mean, geom = "crossbar", width = 0.8, color = "black", fatten = 0.9, position = position_dodge(width = 0.75)) +
  facet_grid(~ corr, labeller = labeller(corr = c(
    "0" = "ρ = 0",
    "0.4" = "ρ = 0.4",
    "0.8" = "ρ = 0.8"
  ))) +
...
# save plot
ggsave("rates_small_study1.png", plot = p_rates_effectsize_study1, width = 10, height = 6, dpi = 300)
```


## where lower worked
```{r}
p_rates_subset_study1 <- combined_rates_by_effect_size_study1 %>%
  semi_join(combined_rates_by_effect_size_study1 %>%
  ...
  mutate(
    ...
    method = dplyr::recode(heuristic,
                    ...
                    "SSVS" = "SSVS"),
    corr = factor(corr, levels = c(0, 0.4, 0.8), labels = c("ρ = 0.0", "ρ = 0.4", "ρ = 0.8")),
...
# save plot
ggsave("rates_study1_subset.png", plot = p_rates_subset_study1, width = 10, height = 12, dpi = 300)
```

```{r}
p_rates_large_medidum_study1 <- combined_rates_by_effect_size_study1 %>%
  filter(!(heuristic %in% c("lower", "thres_lower", "thres_upper"))) %>%
  mutate(
    effect_size = factor(effect_size, levels = c("large", "medium", "small", "null")),
    method = dplyr::recode(heuristic,
  ...
  filter(effect_size %in% c("medium", "large")) %>%
  ggplot(aes(x = factor(TE), y = inclusion_rate, fill = method, col = method)) + 
  geom_boxplot(alpha = 0.7) +
  stat_summary(fun = mean, geom = "crossbar", width = 0.8, color = "black", fatten = 0.9, position = position_dodge(width = 0.75)) +
  facet_grid(effect_size~ corr, labeller = labeller(corr = c(
    "0" = "ρ = 0",
    "0.4" = "ρ = 0.4",
    "0.8" = "ρ = 0.8"
  ), effect_size = c(
...
# save plot
ggsave("rates_large_medium_study1.png", plot = p_rates_large_medidum_study1, width = 10, height = 6, dpi = 300)

p_rates_null_study1 <- combined_rates_by_effect_size_study1 %>%
  filter(!(heuristic %in% c("lower", "thres_lower", "thres_upper"))) %>%
  mutate(
    effect_size = factor(effect_size, levels = c("large", "medium", "small", "null")),
    method = dplyr::recode(heuristic,
...
# save plot
ggsave("rates_null_study1.png", plot = p_rates_null_study1, width = 10, height = 6, dpi = 300)
```
...
# Function to compute PMSE for each heuristic 
...
prj_pred_files_study1 <- list.files("output_study1/ppvs/prj_pred/", pattern = "^prj_pred_", full.names = TRUE)

# Apply function to all files
pmse_results_study1 <- bind_rows(lapply(prj_pred_files_study1, compute_pmse))
pmse_results_study1 <- pmse_results_study1[, -1]

pmse_results_study1 <- pmse_results_study1 %>%
  anti_join(nonconver_reps_study1, by = c("N", "corr", "TE", "r"))

# View results
print(pmse_results_study1)
...
ssvs_pred_files_study1 <- list.files("output_study1/ssvs/ssvs_pred_results/", pattern = "^ssvs_pred_results_N", full.names = TRUE)

# Extract metadata from filenames
ssvs_metadata_study1 <- tibble(
  file = ssvs_pred_files_study1,
  N = as.numeric(str_extract(ssvs_pred_files_study1, "(?<=ssvs_pred_results_N)\\d+")),
  corr = as.numeric(str_extract(ssvs_pred_files_study1, "(?<=_corr)0(?:\\.\\d+)?")),
  TE = str_extract(ssvs_pred_files_study1, "(?<=_TE)[a-z]+"),
  r = as.numeric(str_extract(ssvs_pred_files_study1, "(?<=_r)\\d+"))
)

# Function to compute PMSE
compute_pmse_ssvs_study1 <- function(file_path) {
  load(file_path)  # Loads ssvs_pred_results

  file_info <- ssvs_metadata_study1 %>% filter(file == file_path)
...
ssvs_pmse_results_study1 <- bind_rows(lapply(ssvs_pred_files_study1, compute_pmse_ssvs_study1))

# View results
print(ssvs_pmse_results_study1)
```

# combine pmse
```{r}
# Combine both datasets
combined_pmse_study1 <- bind_rows(pmse_results_study1, ssvs_pmse_results_study1)
combined_pmse_study1$heuristic <- as.factor(combined_pmse_study1$heuristic)

# View structure to check
glimpse(combined_pmse_study1)
print(combined_pmse_study1)
```


```{r}
# Compute means
mean_pmse_study1 <- combined_pmse_study1 %>%
...
pmse_study1 <- combined_pmse_study1 %>%
  filter(!(heuristic %in% c("lower", "thres_lower", "thres_upper"))) %>%
  mutate(
    method = dplyr::recode(heuristic,
                    "default" = "PPVS - SE heuristic upper bound",
                    "SSVS" = "SSVS"),
    corr = factor(corr),    # Force factor
    TE = factor(TE)         # Force factor
  ) %>%
  droplevels() %>%          # Now drop unused levels
  ggplot(aes(x = pmse, fill = method, color = method)) +
  geom_density(alpha = 0.3) +  
  geom_vline(data = mean_pmse_study1,
             aes(xintercept = mean_pmse, color = method),
             linetype = "dashed", size = 0.8) +
  facet_grid(corr ~ TE, labeller = labeller(
      corr = c("0" = "ρ = 0", "0.4" = "ρ = 0.4", "0.8" = "ρ = 0.8"),
      TE = c("clustered" = "Clustered true effects", "mixed" = "Mixed true effects")
    )
  ) +  
  labs(x = "PMSE", y = "Density") +
  theme_classic() +
  theme(
    legend.position = "bottom",
    strip.background = element_rect(colour = "black", fill = "white")
...
combined_pmse_study1 %>%
  filter(!(heuristic %in% c("lower", "thres_lower", "thres_upper"))) %>%
  mutate(
    method = dplyr::recode(heuristic,
                    "default" = "PPVS - SE heuristic upper bound",
                    "SSVS" = "SSVS"),
    facet_label = interaction(
      paste0("ρ = ", corr),
      ifelse(TE == "clustered", "Clustered", "Mixed"),
      sep = ", "
    )
  ) %>%
  ggplot(aes(x = pmse, fill = method, color = method)) +
...
# save plot
ggsave("pmse_study1.png", plot = pmse_study1, width = 10, height = 6, dpi = 300)
```

## pmse where lower worked
```{r}
# filtering the replications where the lower threshold worked
p_pmse_subset_study1 <- combined_pmse_study1 %>%
  ...
  ggplot(aes(x = corr, y = pmse, fill = method, col = method)) +
  geom_boxplot(alpha = 0.7) +  
...
# save plot
ggsave("pmse_subset_study1.png", plot = p_pmse_subset_study1, width = 10, height = 6, dpi = 300)
```
# CI coverage
...
ppvs_summary_coverage_study1 <- bind_rows(lapply(prj_pred_files_study1, compute_coverage))

# remove non-convergence replications
ppvs_summary_coverage_study1 <- ppvs_summary_coverage_study1 %>%
  anti_join(nonconver_reps_study1, by = c("N", "corr", "TE", "r"))

# View results
print(ppvs_summary_coverage_study1)
...
system.time(
  ssvs_coverage_summary_study1 <- bind_rows(lapply(ssvs_pred_files_study1, compute_ssvs_coverage))
)

# View results
print(ssvs_coverage_summary_study1)
```

## combine coverage
```{r}
# Combine both datasets
combined_coverage_summary_study1 <- bind_rows(ppvs_summary_coverage_study1, ssvs_coverage_summary_study1)
combined_coverage_summary_study1$heuristic <- as.factor(combined_coverage_summary_study1$heuristic)

# View structure to check
glimpse(combined_coverage_summary_study1)
print(combined_coverage_summary_study1)
```

## coverage visualization
```{r}
combined_coverage_summary_study1 %>%
...
# Compute means
mean_percen_coverage_study1 <- combined_coverage_summary_study1 %>%
  filter(!(heuristic %in% c("lower", "thres_upper", "thres_lower"))) %>%
  mutate(method = dplyr::recode(heuristic,
                    "default" = "PPVS - SE heuristic upper bound",
                    "SSVS" = "SSVS")) %>%
...
mean_percen_coverage_study1

vline_data <- expand.grid(
  corr = c("0", "0.4", "0.8"),
  TE = c("clustered", "mixed")
) %>%
  filter(!(corr == "0" & TE == "mixed")) %>%  # Exclude the unwanted panel
  mutate(xintercept = 0.95)

p_coverage_percentage_study1 <- combined_coverage_summary_study1 %>%
  filter(!(heuristic %in% c("lower", "thres_upper", "thres_lower"))) %>%
    mutate(method = dplyr::recode(heuristic,
                    "default" = "PPVS - SE heuristic upper bound",
                    "SSVS" = "SSVS")) %>%
  ...
  geom_density(alpha = 0.3) +
  geom_vline(data = mean_percen_coverage_study1,
...
# save plot
ggsave("coverage_study1.png", plot = p_coverage_percentage_study1, width = 10, height = 6, dpi = 300)
```
...
```{r}
# filtering the replications where the lower threshold worked
p_coverage_subset_study1 <- combined_coverage_summary_study1 %>%
  # filter for reps where 'lower' heuristic worked
  semi_join(
    combined_coverage_summary_study1 %>%
  ...
  ggplot(aes(x = corr, y = replication_coverage, fill = method, col = method)) +
  geom_boxplot(alpha = 0.7) +  
  stat_summary(fun = mean, geom = "crossbar", width = 1, color = "black", fatten = 0.9, position = position_dodge(width = 0.75)) +
  facet_grid(~ TE) +  
...
# save plot
ggsave("coverage_subset_study1.png", plot = p_coverage_subset_study1, width = 10, height = 6, dpi = 300)
```
...
```{r}
# Get all time files
time_files_study1 <- list.files("D:/projpred_vs_spikeslab/output_study1/ppvs/time_taken/", pattern = "^time_.*\\.RData$", full.names = TRUE)

# Initialize empty list to collect rows
time_data_study1 <- list()

# Loop through and process each file
for (file in time_files_study1) {
  load(file)  # loads time.taken

  # Extract filename components
  filename <- basename(file)
  N <- as.numeric(str_extract(filename, "(?<=time_N)\\d+"))
  corr <- as.numeric(str_extract(filename, "(?<=_corr)0(?:\\.\\d+)?"))
  TE <- str_extract(filename, "(?<=_TE)[a-zA-Z]+")
  r <- as.numeric(str_extract(filename, "(?<=_r)\\d+"))

  # Append row
  time_data_study1[[length(time_data_study1) + 1]] <- data.frame(
    N = N,
    corr = corr,
    TE = TE,
    r = r,
...
df_time_study1 <- bind_rows(time_data_study1)
df_time_study1
```
...
```{r}
# Get all time files
time_files_ssvs_study1 <- list.files("D:/projpred_vs_spikeslab/output_study1/ssvs/time_taken/", pattern = "^time_.*\\.RData$", full.names = TRUE)

# Initialize empty list to collect rows
time_data_ssvs_study1 <- list()

# Loop through and process each file
for (file in time_files_ssvs_study1) {
  load(file)  # loads time.taken
  ...
  # Append row
  time_data_ssvs_study1[[length(time_data_ssvs_study1) + 1]] <- data.frame(
    N = N,
    corr = corr,
    TE = TE,
    r = r,
...
# Combine all rows into a data frame
df_time_ssvs_study1 <- bind_rows(time_data_ssvs_study1)
df_time_ssvs_study1
```
...
## combined
```{r}
combined_time_study1 <- bind_rows(df_time_study1, df_time_ssvs_study1)
combined_time_study1 %>%
  group_by(N, corr, TE, method) %>%
  summarise(
...
```{r}
# save(combined_rates_study1, combined_rates_by_effect_size_study1, combined_counts_study1, combined_pmse_study1, combined_coverage_summary_study1, combined_time_study1, nonconver_reps_study1, summary_na_study1, file = "result_objects_study1.RData")
```
